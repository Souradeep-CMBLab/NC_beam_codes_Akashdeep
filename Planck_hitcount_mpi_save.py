...
def cos_angle(A,B):
    ...
    if (mag_A * mag_B) == 0:
        return 0 # To handle the case where one the vector becomes Zeros(R_i == Rc)
    cos_theta = dot_product / (mag_A * mag_B)
    cos_theta = np.clip(cos_theta, -1.0, 1.0) # Clip cos_theta to the valid range for arccos to avoid NaNs
    return cos_theta
...
if rank==0:
    hits0=np.zeros(npix)
    cos_2rho0=np.zeros(npix)
else:
    hits=np.zeros(npix)
    cos_2rho=np.zeros(npix)


# time_step=scan_time
scan_time = np.sqrt(4*np.pi/npix)/w3
...
FW=9.651/60 #FWHM in arcmin
e=1.186
S=(FW/np.sqrt(8*np.log(2)))*np.pi/180
sigma_x=np.sqrt(e*S**2)
sigma_y=np.sqrt(S**2/e)

def process_time_step(time_step):#,R,S,pix_ring):

    # 1. Get the vectors
    R,S = get_vectors(time_step)

    # 2. Find the central pixel
    pix = hp.vec2pix(nside, R[0], R[1], R[2])
    ...
    #neighbours = hp.query_disc(nside, Rc , radius=2.25*np.radians(FW))

    Rpi=Rpole-Rc # The line joining the central pixel with pole
    Proj_Rpi=np.cross(Rc,np.cross(Rpi,Rc))
    
    cos_rho=cos_angle(N_t,Proj_Rpi)
    cos_2rho=2*cos_rho**2-1
    sin_2rho=np.sqrt(1-cos_2rho)
    return cos_2rho,pix_ring
    ...
    '''for i, neighbour_pix in enumerate(neighbours):
        ...
        weight[i]=np.exp(-x[i]**2 / (2 * sigma_x**2) -y[i]**2 / (2 * sigma_y**2)) 
        # print(centre[0] )'''
...
start = 0
duration = 60*24*365 # in min (one year)
steps = int(duration / scan_time)
steps_per_rank = int(steps / size)
start_steps = rank * steps_per_rank
end_steps = (rank + 1) * steps_per_rank


if rank==0:
    for i in range(start_steps, end_steps):
        rho, p = process_time_step(i*scan_time)
        cos_2rho0[p]+=rho
        hits0[p]+=1
    np.savetxt("cos_2rho_0.txt", cos_2rho0, delimiter=",")
    np.savetxt("hit_count_0.txt", hits0, delimiter=",")
else:
    for i in range(start_steps, end_steps):
        rho, p = process_time_step(i*scan_time)
        cos_2rho[p]+=rho
        hits[p]+=1
    np.savetxt(f"cos_2rho_{rank}.txt", cos_2rho, delimiter=",")
    np.savetxt(f"hit_count_{rank}.txt", hits, delimiter=",")
'''if rank!=0:
    comm.send(hits, dest=0)
if rank==0:
    for i in range(1,size):
        hits=np.empty(npix,dtype='float64')
        comm.recv(hits,source=i)
        hits0+=hits
    filename = f"hit_count{nside}.txt"
    np.savetxt(filename, hits0, delimiter=",")
if rank!=0:
    comm.send(cos_2rho, dest=0)
if rank==0:
    for i in range(1,size):
        cos_2rho=np.empty(npix,dtype='float64')
        comm.recv(cos_2rho,source=i)
        cos_2rho0+=cos_2rho
    filename2 = f"cos_2rho{nside}.txt"
    np.savetxt(filename2, cos_2rho0, delimiter=",")
if rank==0:
    end_time=time.time()
    print("total_time=",(end_time-start_time)/60,"minutes")
comm.Barrier()
...
def process_chunk(chunk_range,steps1,length): #start1,duration1,
    ...
    for time_period in time_periods:#, desc=f"Processing chunk {chunk_range[0]} to {chunk_range[1]}"):
...
print("successfully read the chunks file in rank",rank)
mpi_id=int(sys.argv[1])
chunk_id=0+mpi_id*16+rank
if chunk_id==0:
    chunk_ranges = [0,int(Chunk_cuts[0])]
else:
    chunk_ranges = [int(Chunk_cuts[chunk_id-1]),int(Chunk_cuts[chunk_id])]
    
process_chunk(chunk_ranges,steps,length)#start,duration,
